...
class MockPolicy:
    def evaluate(self, _ctx, _intent, _req):
        return pytest.importorskip("tollgate").Decision(
            decision=DecisionType.ASK, reason="Needs approval"
        )
...
async def test_audit_redaction():
    ...
    intent = Intent("i", "r")
    req = ToolRequest("t", "a", "res", Effect.WRITE, {"secret": "12345", "public": "hi"})
...
async def test_approver_timeout_behavior():
    class TimeoutApprover:
        async def request_approval_async(self, *args):
    ...
    from tollgate import TollgateApprovalDenied
